...
$input_membershipship_id = $membershipship_id = "";
...
if (!empty($register_date)) {
    ...
    if (empty($_POST['Contact number'])) {
        $Contact number_err = 'Phone number is required';
    } else {
        $Contact number = filter_input(INPUT_POST, 'Contact number', FILTER_SANITIZE_STRING);
    }
    ...
    // Sanitize membershipship and staff IDs
    $membershipship_id = $_POST['membershipship_id'] ?? null;
    $staff_id = $_POST['staff_id'] ?? null;

    // If there are no errors, insert the data into the database
    if (empty($email_err) && empty($register_date_err) && empty($Contact number_err) && empty($password_err)) {
        $link = new mysqli("localhost", "root", "", "cafedb");
        if ($link->connect_error) {
            die("Connection failed: " . $link->connect_error);
        }

$insert_query = "INSERT INTO membershipss  (email, register_date, Contact number, password, membershipship_id, staff_id) VALUES (?, ?, ?, ?, ?, ?)";
$stmt = $link->prepare($insert_query);
$stmt->bind_param("ssssss", $email, $register_date, $Contact number, $password, $membershipship_id, $staff_id);

        if ($stmt->execute()) {
            // Success, redirect to success page or do something else
            header("location: success_create_member_memberships.php");
            exit();
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Create New Member memberships</title>
...
<div class="wrapper">
    <h1>Northside </h1>
    <h3>Create New Member memberships</h3>
    <p>Please fill in memberships Information Properly</p>

    <form method="POST" action="success_create_member_memberships.php" class="ht-600 w-50">
        ...
        <div class="form-group">
            ...
            <input placeholder="+013 985 3921" type="text" name="Contact number" id="Contact number" required class="form-control <?php echo $Contact number_err ? 'is-invalid' : ''; ?>" value="<?php echo $Contact number; ?>"><br>
            <div class="invalid-feedback">
                <?php echo $Contact number_err; ?>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input placeholder="1" min=1 type="number" name="user_id" id="user_id" class="form-control" value="<?php echo $user_id; ?>">
        </div>

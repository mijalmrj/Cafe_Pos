...
if (file_exists('setup_completed.flag')) {
    ...
    // Create the 'cafedb' database if it doesn't exist
    $sqlCreateDB = "CREATE DATABASE IF NOT EXISTS cafedb";
    if ($link->query($sqlCreateDB) === TRUE) {
        echo "Database 'cafedb' created successfully.<br>";
    } else {
        echo "Error creating database: " . $link->error . "<br>";
    }

    // Switch to using the 'cafedb' database
    $link->select_db('cafedb');

    // Execute SQL statements from "cafedb.txt"
    ...
    // Execute SQL statements from "cafedb.txt"
    executeSQLFromFile('cafedb.txt', $link);

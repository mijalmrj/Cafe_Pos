...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // Insert Data into membershipss Table
        $insert_memberships_query = "INSERT INTO membershipss (memberships_id, email, register_date, Contact number, password) VALUES (?, ?, ?, ?, ?)";
        $stmt_memberships = $link->prepare($insert_memberships_query);
        $stmt_memberships->bind_param("issss", $memberships_id, $email, $register_date, $Contact number, $password);

        // Execute the query to insert data into membershipss table
        if (!$stmt_memberships->execute()) {
            throw new Exception("Error creating memberships: " . $stmt_memberships->error);
        }
        ...
        // Execute the query to insert data into membershipships table
        if (!$stmt_membershipship->execute()) {
            throw new Exception("Error creating membershipship: " . $stmt_membershipship->error);
        }

        // Commit the transaction if everything is successful
        $link->commit();

        $message = "membershipship created successfully.";
        ...
        $stmt_memberships->close();
        $stmt_membershipship->close();
